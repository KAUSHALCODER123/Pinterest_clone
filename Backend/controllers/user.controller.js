...
export const registerUser = async (req, res, next) => {
  ...
  const { name, email, password } = req.body;
    ...
    // Auto-generate a username if not provided (use name or email prefix)
    let baseUsername = '';
    if (req.body.username) baseUsername = String(req.body.username);
    else if (name) baseUsername = name.trim().toLowerCase().replace(/\s+/g, '');
    else baseUsername = email.split('@')[0];

    // Append a small random suffix to reduce collision chances
    ...
    try {
      ...
      if (saveError.name === 'ValidationError') {
        const firstKey = Object.keys(saveError.errors)[0];
        const message = saveError.errors[firstKey].message || 'Validation failed';
